...
def execute_queries():
    ...
    cursor.execute("""
        SELECT Departments.name, AVG(Positions.salary) AS avg_salary FROM Employees
    ...
    cursor.execute("""
        SELECT Departments.name, COUNT(Employees.employee_id) AS employee_count FROM Employees
        JOIN Departments ON Employees.dept_id = Departments.dept_id
        GROUP BY Departments.name;
    """)
    ...
    cursor.execute("""
        SELECT last_name, first_name, salary, (salary * bonus_percentage / 100) AS bonus FROM Employees
        JOIN Positions ON Employees.position_id = Positions.position_id;
    """)
    ...
    cursor.execute("""
        SELECT Departments.name, education, COUNT(Employees.employee_id) AS employee_count FROM Employees
        JOIN Departments ON Employees.dept_id = Departments.dept_id
